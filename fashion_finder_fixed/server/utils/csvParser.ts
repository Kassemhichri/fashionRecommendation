...
interface MergedProduct extends StyleData {
  imageUrl: string;
}
 ...
 */
export function getMergedProducts(): MergedProduct[] {
  ...
  const styles = parseCSV(stylesPath) as StyleData[];
  ...
  const productImageMap = new Map<string, boolean>();
  ...
  const mergedProducts: MergedProduct[] = styles.map((style) => {
